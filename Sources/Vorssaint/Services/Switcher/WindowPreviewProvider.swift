...
final class WindowPreviewProvider {
    ...
    func refreshPreviews(for items: [SwitcherItem],
        ...
        captureTask = Task(priority: .userInitiated) { [weak self] in
            ...
            // ScreenCaptureKit screenshot fallback needs macOS 14+ APIs absent
            // from the Xcode 14 SDK used to build for macOS 12.
        }
    }
    ...
    private func scheduleWarm(pid: pid_t) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.9) { [weak self] in
            guard let self = self, self.pendingWarmPid == pid, self.activationToken != nil else { return }
            self.pendingWarmPid = nil
            let items = WindowEnumerator.listWindows(for: pid)
            guard !items.isEmpty else { return }
            self.warmTask?.cancel()
            self.warmTask = Task(priority: .utility) { [weak self] in
                guard let self = self else { return }
                for item in items {
