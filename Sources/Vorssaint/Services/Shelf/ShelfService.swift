...
final class ShelfService: ObservableObject {
    ...
    func syncDockedShelf() {
        ...
        if panel.contentViewController == nil {
            let host = SizingHostingController(rootView: DockedShelfView().environmentObject(self))
    ...
    private func ensurePanel() -> NSPanel {
        ...
        panel.isReleasedWhenClosed = false
        panel.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .ignoresCycle]
        let host = SizingHostingController(rootView: ShelfView().environmentObject(self))
        host.applyPreferredContentSizing()
        panel.contentViewController = host
        self.panel = panel
        return panel
