...
private final class MediaResultHolder<T> {
...
final class MediaService: ObservableObject {
    ...
    private func runAsync<T>(_ operation: @escaping () async throws -> T) throws -> T {
        ...
        let holder = MediaResultHolder<T>()

        Task {
            do {
                holder.value = .success(try await operation())
            } catch {
                holder.value = .failure(error)
            }
            semaphore.signal()
        }

        semaphore.wait()
        guard let result = holder.value else {
