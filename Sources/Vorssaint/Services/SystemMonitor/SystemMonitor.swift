...
final class SystemMonitor: ObservableObject {
    ...
    func panelDidAppear() {
        runOnMain { monitor in
            monitor.panelClients += 1
            monitor.ensureTimer()
            monitor.refresh(suppressImmediateGPU: true)
            monitor.scheduleDeferredGPURefreshIfNeeded()
        }
    }
    ...
    func panelDidDisappear() {
        runOnMain { monitor in
            monitor.panelClients = max(0, monitor.panelClients - 1)
            monitor.stopTimerIfIdle()
        }
    }
    ...
    func setMenuPanelNeeds(_ needs: SystemMonitorPanelNeeds) {
        runOnMain { monitor in
            if needs == monitor.menuPanelNeeds {
            ...
            let previousNeeds = monitor.menuPanelNeeds
            let neededGPUBefore = monitor.currentPlan(defaults: defaults).needGPUUsage
            monitor.menuPanelNeeds = needs
            let needsGPUAfter = monitor.currentPlan(defaults: defaults).needGPUUsage
            if monitor.shouldRun {
                monitor.ensureTimer()
                ...
                monitor.refresh(suppressImmediateGPU: suppressImmediateGPU)
    ...
    func suppressGPUReadsForTransientUI(duration: TimeInterval = 0.9) {
        runOnMain { monitor in
            ...
            monitor.suppressGPUReadsUntil = max(monitor.suppressGPUReadsUntil, until)
            if monitor.shouldSample(), monitor.currentPlan(defaults: .standard).needGPUUsage {
                monitor.scheduleDeferredGPURefreshIfNeeded()
            }
        }
    }
    ...
    func setMenuBarActive(_ active: Bool) {
        runOnMain { monitor in
            guard active != monitor.menuBarActive else {
                if active { monitor.resyncIfPlanChanged() }
                return
            }
            monitor.menuBarActive = active
            if active {
                monitor.ensureTimer()
                monitor.refresh()
            } else {
    ...
    func setAlertsActive(_ active: Bool) {
        runOnMain { monitor in
            guard active != monitor.alertsActive else {
                if active { monitor.resyncIfPlanChanged() }
                return
            }
            monitor.alertsActive = active
            if active {
                monitor.ensureTimer()
                monitor.refresh()
            } else {
    ...
    func setInterval(seconds: Int) {
        runOnMain { monitor in
            ...
            guard clamped != monitor.intervalSeconds else { return }
            monitor.intervalSeconds = clamped
            ...
            monitor.syncTimerCadence(plan: monitor.currentPlan(defaults: .standard))
            if monitor.timer != nil { monitor.restartTimer() }
        }
    }

    private func runOnMain(_ work: @escaping (SystemMonitor) -> Void) {
