...
struct QuickLauncherView: View {
    ...
    var body: some View {
        ...
        .onChange(of: launcher.presentationID) { _ in
            hoveredItem = nil
            draggingItem = nil
        }
        .onChange(of: launcher.activeUtility) { _ in
            launcher.refreshPanelLayout()
        }
        .onChange(of: launcher.isEditing) { _ in
        ...
        .onChange(of: launcher.editingOptionsItem) { _ in
            launcher.refreshPanelLayout()
        }
        .onChange(of: clipboardEnabled) { _ in
            launcher.refreshPanelLayout()
        }
        .onChange(of: colorFormat) { _ in
            launcher.refreshPanelLayout()
        }
    }
        ...
        Button {
            ...
            VStack(spacing: 7) {
                ...
                Text(title(for: item))
                    ...
                    .multilineTextAlignment(.center)
                    .lineLimitCompat(2, reservesSpace: true)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    @ViewBuilder
    private var hiddenItemsLayout: some View {
    ...
    private func hiddenItemChip(_ item: QuickLauncherItem) -> some View {
        Button {
            withAnimation(.easeOut(duration: 0.15)) {
