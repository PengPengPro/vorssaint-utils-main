...
struct HomebrewSettings: View {
    ...
    var body: some View {
        ...
        .onChange(of: query) { _ in resetContextSelection() }
        .onChange(of: searchKind) { _ in resetContextSelection() }
        .onChange(of: installedFilter) { _ in clearSelectionIfHidden() }
        .onChange(of: homebrew.operationStatus?.targetID) { _ in
