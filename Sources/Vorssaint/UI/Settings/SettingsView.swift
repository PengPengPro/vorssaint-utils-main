...
struct SettingsView: View {
    ...
    private var sidebarSections: [(title: String, items: [SettingsSidebarItem])] {
        [
            (categories.essentials, [
                SettingsSidebarItem(page: .general, title: l10n.s.tabGeneral, icon: "gearshape",
                            keywords: [l10n.s.launchAtLogin, l10n.s.languageLabel, l10n.s.showMenuBarIcon]),
                SettingsSidebarItem(page: .energy, title: l10n.s.tabEnergy, icon: "bolt.fill",
                ...
                SettingsSidebarItem(page: .monitor, title: l10n.s.tabMonitor, icon: "chart.line.uptrend.xyaxis",
                            keywords: [l10n.s.menuBarSpacingLabel, l10n.s.menuBarHideIconToggle,
                                       ...
                                       l10n.s.monitorMenuBarMetricColor]),
            ]),
            (categories.windowsControls, [
                ...
                SettingsSidebarItem(page: .switcher, title: l10n.s.tabSwitcher, icon: "rectangle.on.rectangle",
                            keywords: [l10n.s.switcherEnable, l10n.s.dockClickMinimize,
                                       l10n.s.dockClickCycleWindows]),
                SettingsSidebarItem(page: .windowLayout, title: FeatureStrings.windowLayout(l10n.language).title, icon: "rectangle.3.group",
                            keywords: [l10n.s.dockClickCycleWindows]),
                SettingsSidebarItem(page: .autoQuit, title: l10n.s.autoQuitName, icon: "xmark.rectangle",
                            keywords: [l10n.s.autoQuitEnable]),
            ]),
            (categories.files, [
                SettingsSidebarItem(page: .clipboard, title: FeatureStrings.clipboard(l10n.language).title, icon: "doc.on.clipboard",
                            keywords: [FeatureStrings.clipboard(l10n.language).limit,
                                       FeatureStrings.clipboard(l10n.language).skipSensitive]),
                SettingsSidebarItem(page: .cutPaste, title: l10n.s.cutPasteName, icon: "scissors",
                            keywords: [l10n.s.cutPasteEnable]),
                SettingsSidebarItem(page: .shelf, title: l10n.s.shelfName, icon: "tray.full",
                            keywords: [l10n.s.shelfEnable, l10n.s.shelfDropZoneToggle]),
                SettingsSidebarItem(page: .media, title: l10n.s.mediaName, icon: "photo.on.rectangle.angled",
                            keywords: ["PDF", "GIF", l10n.s.mediaStartConvertPDF, l10n.s.ocrName]),
            ]),
            (categories.utilities, [
                SettingsSidebarItem(page: .quickTools, title: l10n.s.quickToolsTab, icon: "wand.and.rays",
                ...
                SettingsSidebarItem(page: .urlCleaner, title: l10n.s.urlCleanerName, icon: "link"),
                SettingsSidebarItem(page: .homebrew, title: l10n.s.homebrewName, icon: "shippingbox"),
                SettingsSidebarItem(page: .uninstaller, title: l10n.s.uninstallerName, icon: "trash"),
                SettingsSidebarItem(page: .keyDebounce, title: l10n.s.keyDebounceName, icon: "keyboard"),
            ]),
            (categories.app, [
                SettingsSidebarItem(page: .shortcuts, title: l10n.s.shortcutsPageTitle, icon: "command",
                            keywords: [l10n.s.hotkeyToggle]),
                SettingsSidebarItem(page: .advanced, title: l10n.s.tabAdvanced, icon: "wrench.and.screwdriver"),
                SettingsSidebarItem(page: .about, title: l10n.s.tabAbout, icon: "info.circle",
                            keywords: [l10n.s.reviewIntro]),
                SettingsSidebarItem(page: .releaseNotes, title: l10n.s.tabReleaseNotes, icon: "sparkles"),
                SettingsSidebarItem(page: .support, title: l10n.s.tabSupport, icon: "heart.fill"),
            ]),
        ]
    }
    ...
    var body: some View {
        Group {
            if #available(macOS 13.0, *) {
                NavigationSplitView {
                    SettingsSidebarModern(
                        searchQuery: $searchQuery,
                        sidebarSections: sidebarSections,
                        selection: $router.page
                    )
                ...
                HSplitView {
                    settingsSidebarLegacy
                        .frame(minWidth: 198, idealWidth: 210, maxWidth: 240)
                    legacyDetailPane
                        .frame(minWidth: 480)
                }
            }
    ...
    private var settingsSidebarLegacy: some View {
        List {
            legacySearchField
            settingsSidebarRowsLegacy
        }
        .listStyle(.sidebar)
    }
    ...
    private var legacySearchField: some View {
        HStack(spacing: 6) {
            ...
            TextField(l10n.s.settingsSearchPlaceholder, text: $searchQuery)
                .textFieldStyle(.plain)
        }
        .padding(.vertical, 4)
    }

    /// Pages that already manage their own scrolling or full-height layout.
    private var legacyDetailManagesScrolling: Bool {
        switch router.page {
        case .releaseNotes, .support, .homebrew, .uninstaller:
            return true
        default:
            return false
        }
    ...
    @ViewBuilder
    private var legacyDetailPane: some View {
        let content = detail
            .frame(maxWidth: .infinity, alignment: .topLeading)

        if legacyDetailManagesScrolling {
            content
    ...
    @ViewBuilder
    private var settingsSidebarRowsLegacy: some View {
            ...
            if !items.isEmpty {
                Section(section.title) {
                    ForEach(items) { item in
                        Button {
                            router.page = item.page
                        } label: {
                            Label(item.title, systemImage: item.icon)
                                .frame(maxWidth: .infinity, alignment: .leading)
                        }
                        .buttonStyle(SettingsSidebarButtonStyle(isSelected: router.page == item.page))
                    }
                }
            }
        }
...
private struct SettingsSidebarModern: View {
    @ObservedObject private var l10n = L10n.shared
    @Binding var searchQuery: String
    let sidebarSections: [(title: String, items: [SettingsSidebarItem])]
    ...
    var body: some View {
        ...
        .listStyle(.sidebar)
        .searchable(text: $searchQuery, prompt: l10n.s.settingsSearchPlaceholder)
    ...
    var body: some View {
        Section(l10n.s.updatesSection) {
            Toggle(l10n.s.autoCheckToggle, isOn: $autoCheck)
                .onChange(of: autoCheck) { value in
            ...
            Section(l10n.s.extraBrightnessName) {
                if extraBrightness.supported {
                    Toggle(l10n.s.extraBrightnessName, isOn: $extraBrightnessEnabled)
                        .onChange(of: extraBrightnessEnabled) { _ in
        ...
        Form {
            Section(l10n.s.scrollSection) {
                Toggle(l10n.s.invertMouseScroll, isOn: $inverterEnabled)
                    .onChange(of: inverterEnabled) { _ in
            ...
            Section(l10n.s.smoothScrollName) {
                Toggle(l10n.s.smoothScrollName, isOn: $smoothScrollEnabled)
                    .onChange(of: smoothScrollEnabled) { _ in
            ...
            Section(l10n.s.middleClickSection) {
                Toggle(l10n.s.middleClickEnable, isOn: $middleClickEnabled)
                    .onChange(of: middleClickEnabled) { _ in
                ...
                if middleClickEnabled {
                    ...
                    .onChange(of: middleClickTapFingers) { _ in
                        MiddleClickService.shared.syncWithPreferences()
                    }
...
struct SwitcherSettings: View {
    ...
    var body: some View {
        Form {
            switcherOptionsSection
            dockPreviewOptionsSection
            previewSizeSection
            switcherPermissionSections
        }
        .groupedFormStyle()
    }

    @ViewBuilder
    private var switcherOptionsSection: some View {
        Section {
            switcherCoreToggles
        } header: {
    ...
    @ViewBuilder
    private var switcherCoreToggles: some View {
        Group {
            Toggle(l10n.s.switcherEnable, isOn: $switcherEnabled)
                .onChange(of: switcherEnabled) { _ in
    ...
    @ViewBuilder
    private var dockPreviewOptionsSection: some View {
        Section(l10n.s.dockPreviewName) {
            Toggle(l10n.s.dockPreviewEnable, isOn: $dockPreviewEnabled)
                .onChange(of: dockPreviewEnabled) { _ in
                    DockPreviewService.shared.syncWithPreferences()
                }
            Text(dockPreviewCaption)
                .font(.caption)
                .foregroundStyle(dockPreviewWarning ? Color.orange : Color.secondary)
            Toggle(l10n.s.dockClickMinimize, isOn: $dockClickMinimize)
                .onChange(of: dockClickMinimize) { _ in
            ...
            Toggle(l10n.s.dockClickCycleWindows, isOn: $dockClickCycleWindows)
                .onChange(of: dockClickCycleWindows) { _ in
                    DockClickService.shared.syncWithPreferences()
                }
    ...
    private var previewSizeSection: some View {
        Section(l10n.s.previewSizeLabel) {
    ...
    @ViewBuilder
    private var switcherPermissionSections: some View {
...
private struct SettingsSidebarButtonStyle: ButtonStyle {
    ...
    private func backgroundColor(isPressed: Bool) -> Color {
        if isSelected {
            return Color.accentColor.opacity(isPressed ? 0.28 : 0.18)
        }
        if isPressed {
