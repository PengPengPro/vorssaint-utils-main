...
extension Notification.Name {
    ...
    static let menuPanelDidClose = Notification.Name("VorssaintMenuPanelDidClose")
}
...
struct MenuPanelView: View {
    ...
    @State private var contentHeightSection: PanelSectionID = .keepAwake
    @State private var selectedMetric: MetricDetailKind?
    @State private var lastPublishedPopoverSize = CGSize.zero

    /// macOS 12 cannot resize NSPopover smoothly per tab; keep one stable
    /// height and scroll inside so section switches do not flicker.
    private var usesStableLegacyPopoverHeight: Bool {
        if #available(macOS 13.0, *) { return false }
        return true
    }

    private var legacyStableNavigableContentHeight: CGFloat {
        visibleSections.map { estimatedContentHeight(for: $0) }.max() ?? 360
    }

    private var legacyStableNavigablePanelHeight: CGFloat {
        min(maxHeight, max(220, legacyStableNavigableContentHeight + navigableChromeHeight))
    }
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .menuPanelDidClose)) { _ in
            guard !panelFocus.isSwitchingMetricAnchor else { return }
            selectedMetric = nil
            navigableContentHeight = 0
            metricContentHeight = 0
            lastPublishedPopoverSize = .zero
        }
        .onDisappear {
        ...
        .onChange(of: selectedSection) { section in
            contentHeightSection = section
            guard !usesStableLegacyPopoverHeight else { return }
            navigableContentHeight = 0
            publishPopoverSize()
        }
        ...
        .onChange(of: updateBannerHeight) { _ in publishPopoverSize() }
        .onChange(of: navigableContentHeight) { height in
            guard height > 1 else { return }
            contentHeightSection = activeSection
            if !usesStableLegacyPopoverHeight {
                publishPopoverSize()
            }
        }
        .onChange(of: metricContentHeight) { _ in publishPopoverSize() }
        .onChange(of: monitorNeeds) { _ in
            syncMonitorSampling()
        }
        .onChange(of: updates.state) { state in
            if !state.showsMenuPanelBanner {
                updateBannerHeight = 0
            }
        }
        .onChange(of: panelFocus.request) { request in
            applyFocus(request)
        }
    }
    ...
    private func publishPopoverSize() {
        ...
        lastPublishedPopoverSize = size
        NotificationCenter.default.post(
            name: .menuPanelContentSizeChanged,
            object: nil,
    ...
    private var navigablePanel: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            OverlayScrollView(measuredHeight: $navigableContentHeight, resetToken: activeSection) {
                VStack(alignment: .leading, spacing: 12) {
    ...
    private var navigableScrollHeight: CGFloat {
        if usesStableLegacyPopoverHeight {
            return max(80, legacyStableNavigablePanelHeight - navigableChromeHeight)
        }
        let estimate = estimatedNavigableContentHeight
        let measured = (navigableContentHeight > 1 && contentHeightSection == activeSection)
            ? navigableContentHeight
            : estimate
    ...
    private var navigablePanelHeight: CGFloat {
        if usesStableLegacyPopoverHeight {
            return legacyStableNavigablePanelHeight
        }
        return min(maxHeight, max(220, navigableScrollHeight + navigableChromeHeight))
    }
    ...
    private var estimatedNavigableContentHeight: CGFloat {
        estimatedContentHeight(for: activeSection)
    }

    private func estimatedContentHeight(for section: PanelSectionID) -> CGFloat {
        switch section {
...
struct UtilitiesSection: View {
    ...
    var body: some View {
        ...
        .onChange(of: showHomebrewPanel) { shown in
            if shown {
        ...
        .onChange(of: showMediaPanel) { shown in
            if shown {
                PanelInteractionState.shared.keepsPopoverOpen = true
        ...
        .onChange(of: showClipboardPanel) { shown in
            if shown {
                PanelInteractionState.shared.keepsPopoverOpen = true
        ...
        .onChange(of: showWindowLayoutPanel) { shown in
            if shown {
                PanelInteractionState.shared.keepsPopoverOpen = true
...
struct QuickControlsSection: View {
    ...
    var body: some View {
        PanelSection(.controls, title: l10n.s.quickControlsSection, collapsible: collapsible,
            ...
            .onAppear {
                syncExpandedFromDefaults()
                MiddleClickService.shared.refreshDragGestureConflict()
            }
        }
    }
    
    private func syncExpandedFromDefaults() {
        windowsExpanded = UserDefaults.standard.bool(forKey: DefaultsKey.panelControlWindowsExpanded)
        inputExpanded = UserDefaults.standard.bool(forKey: DefaultsKey.panelControlInputExpanded)
        filesExpanded = UserDefaults.standard.bool(forKey: DefaultsKey.panelControlFilesExpanded)
    }
    ...
    private func setExpanded(_ category: ControlCategory, _ expanded: Bool) {
        switch category {
        case .windows:
            windowsExpanded = expanded
            UserDefaults.standard.set(expanded, forKey: DefaultsKey.panelControlWindowsExpanded)
        case .inputDevices:
            inputExpanded = expanded
            UserDefaults.standard.set(expanded, forKey: DefaultsKey.panelControlInputExpanded)
        case .files:
            filesExpanded = expanded
            UserDefaults.standard.set(expanded, forKey: DefaultsKey.panelControlFilesExpanded)
        }
    }
    ...
    private func itemView(_ item: ControlPanelItem, editing: Bool) -> some View {
        ...
        case .mouseScroll:
            PanelToggleRow(title: l10n.s.invertMouseScroll,
                ...
                .onChange(of: scrollEnabled) { enabled in
        ...
        case .switcher:
            VStack(alignment: .leading, spacing: 5) {
                PanelToggleRow(title: l10n.s.switcherSection,
                    ...
                    .onChange(of: switcherEnabled) { enabled in
        ...
        case .keyDebounce:
            VStack(alignment: .leading, spacing: 5) {
                PanelToggleRow(title: l10n.s.keyDebounceName,
                    ...
                    .onChange(of: keyDebounceEnabled) { enabled in
        ...
        case .cutPaste:
            PanelToggleRow(title: l10n.s.cutPasteName,
                ...
                .onChange(of: cutPasteEnabled) { enabled in
        ...
        case .autoQuit:
            PanelToggleRow(title: l10n.s.autoQuitName,
                ...
                .onChange(of: autoQuitEnabled) { enabled in
        ...
        case .windowMaximize:
            PanelToggleRow(title: l10n.s.windowMaximizeName,
                ...
                .onChange(of: windowMaximizeEnabled) { enabled in
        ...
        case .dockPreview:
            PanelToggleRow(title: l10n.s.dockPreviewName,
                ...
                .onChange(of: dockPreviewEnabled) { enabled in
        ...
        case .dockClick:
            PanelToggleRow(title: l10n.s.dockClickMinimize,
                ...
                .onChange(of: dockClickEnabled) { enabled in
        ...
        case .dockClickCycle:
            PanelToggleRow(title: l10n.s.dockClickCycleWindows,
                ...
                .onChange(of: dockClickCycleEnabled) { enabled in
                    DockClickService.shared.syncWithPreferences()
                    requestAccessibilityIfNeeded(enabled)
                }
        case .middleClick:
            PanelToggleRow(title: l10n.s.middleClickEnable,
                ...
                .onChange(of: middleClickEnabled) { enabled in
    ...
    private func resetPanelDefaults() {
        ...
        inputExpanded = false
        filesExpanded = false
        UserDefaults.standard.set(false, forKey: DefaultsKey.panelControlWindowsExpanded)
        UserDefaults.standard.set(false, forKey: DefaultsKey.panelControlInputExpanded)
        UserDefaults.standard.set(false, forKey: DefaultsKey.panelControlFilesExpanded)
    }
        ...
        VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 8) {
                ...
                Toggle("", isOn: $switcherIconRowMode)
                    ...
                    .toggleStyle(.switch)
                    .controlSize(.mini)
                    .onChange(of: switcherIconRowMode) { _ in
                        AppSwitcher.shared.syncWithPreferences()
                    }
            }
...
private struct OverlayScrollView<Content: View>: NSViewRepresentable {
    ...
    func updateNSView(_ scroll: NSScrollView, context: Context) {
        ...
        if let token = resetToken, token != context.coordinator.lastResetToken {
            context.coordinator.lastResetToken = token
            scrollToTop(scroll)
        }
    ...
    private func scrollToTop(_ scroll: NSScrollView) {
        ...
        let y = max(0, document.bounds.height - clipHeight)
    ...
    final class Coordinator {
        ...
        var lastResetToken: AnyHashable?
    }
}
