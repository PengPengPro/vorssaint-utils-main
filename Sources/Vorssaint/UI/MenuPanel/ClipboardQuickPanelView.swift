...
struct ClipboardQuickPanelView: View {
    ...
    var body: some View {
        ...
        .onChange(of: history.quickWindowPresentationID) { _ in
            hoveredEntryID = nil
        }
    }
    ...
    private var content: some View {
        if filtered.isEmpty {
            ...
            ScrollViewReader { proxy in
                ...
                .onChange(of: history.quickSelectionIndex) { _ in
                    scrollSelectedEntry(with: proxy)
                }
                .onChange(of: history.quickSelectionIsVisible) { _ in
                    scrollSelectedEntry(with: proxy)
                }
                .onChange(of: history.quickQuery) { _ in
                    scrollSelectedEntry(with: proxy)
                }
            }
        }
