...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, NSWindowDelegate {
    ...
    private var popoverIsResizing = false
    private var pendingPopoverSize: CGSize?
    private var popoverSizeWorkItem: DispatchWorkItem?
    private var lastAppliedPopoverSize = CGSize.zero
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(menuPanelContentSizeChanged),
    ...
    private func togglePopover(anchor button: NSStatusBarButton? = nil,
        ...
        showPopover(anchor: button, allowRecentClose: allowRecentClose)
    }
    ...
    private func toggleMainPopover() {
        ...
        togglePopover(allowRecentClose: true)
    }
    ...
    @objc private func appResignedActive() {
        ...
        if popover.isShown, !PanelInteractionState.shared.keepsPopoverOpen {
            guard !isMouseInPopover(at: NSEvent.mouseLocation) else { return }
    ...
    func isMouseInPopover(at point: NSPoint) -> Bool {
        guard popover.isShown,
              let frame = popover.contentViewController?.view.window?.frame else { return false }
    ...
    private func schedulePopoverSize(_ size: CGSize) {
        pendingPopoverSize = size
        popoverSizeWorkItem?.cancel()
        if lastAppliedPopoverSize == .zero {
            let size = pendingPopoverSize
            pendingPopoverSize = nil
            if let size {
                applyPopoverSizePreservingTop(size)
            }
            return
        }
        let work = DispatchWorkItem { [weak self] in
            guard let self, let size = self.pendingPopoverSize else { return }
            self.pendingPopoverSize = nil
            self.applyPopoverSizePreservingTop(size)
        }
        popoverSizeWorkItem = work
    ...
    private func applyPopoverSizePreservingTop(_ size: CGSize) {
        ...
        if abs(size.height - lastAppliedPopoverSize.height) < 0.5,
           abs(size.width - lastAppliedPopoverSize.width) < 0.5 {
            return
        }
    ...
    func showOnboarding(mode: OnboardingMode = .full) {
        ...
        let host = SizingHostingController(rootView: OnboardingView(mode: mode) { [weak self] in
            self?.markOnboardingComplete()
            Notifier.requestPermission()
            self?.onboardingWindow?.close()
        })
        host.applyPreferredContentSizing()
        let window = NSWindow(contentViewController: host)
    ...
    private func showUpdateShowcaseIntro() {
        ...
        let host = SizingHostingController(rootView: UpdateShowcaseIntroView(
    ...
    private func showSupportUpdateIntro() {
        ...
        let host = SizingHostingController(rootView: UpdateSupportIntroView(
            onClose: { [weak self] in
    ...
    private func showDockPreviewIntro() {
        ...
        let host = SizingHostingController(rootView: DockPreviewIntroView(
    ...
    func showUpdatePreview() {
        ...
        let host = SizingHostingController(rootView: UpdatePreviewView(
            version: version,
