...
# macOS 15+ / Xcode 16+ codesign; absent from Xcode 14 (xattr -c -r handles cleanup).
CODESIGN_STRIP_XATTRS=()
if codesign --help 2>&1 | grep -q 'strip-disallowed-xattrs'; then
    CODESIGN_STRIP_XATTRS=(--strip-disallowed-xattrs)
fi
...
finalize_installed_bundle_after_child() {
    ...
    sleep 3
    if [[ -n "$devid" ]]; then
        /usr/bin/codesign --force "${CODESIGN_STRIP_XATTRS[@]}" --options runtime --timestamp \
        ...
        /usr/bin/codesign --force "${CODESIGN_STRIP_XATTRS[@]}" --sign "$LEGACY_IDENTITY" "$bundle"
    else
        /usr/bin/codesign --force "${CODESIGN_STRIP_XATTRS[@]}" --sign - "$bundle"
    fi
...
CORE_AUDIO_SHIM="Sources/Vorssaint/Services/Audio/CoreAudioProcessAPI"
PROCESS_USAGE_SHIM="Sources/Vorssaint/Services/SystemMonitor/ProcessUsageShim"
LAUNCH_AT_LOGIN_SHIM="Sources/Vorssaint/Core/LaunchAtLoginShim"
OBJC_SHIMS_HEADER="Sources/Vorssaint/Support/ObjCShims.h"

compile_objc_shims() {
    clang -c -target "$TARGET" -isysroot "$SDK" \
        -I Sources/Vorssaint/Services/Audio \
        -I Sources/Vorssaint/Services/SystemMonitor \
        -o build/CoreAudioProcessAPI.o \
        "$CORE_AUDIO_SHIM.m"
    clang -c -target "$TARGET" -isysroot "$SDK" \
        -I Sources/Vorssaint/Services/SystemMonitor \
        -o build/ProcessUsageShim.o \
        "$PROCESS_USAGE_SHIM.m"
    clang -c -target "$TARGET" -isysroot "$SDK" \
        -o build/LaunchAtLoginShim.o \
        "$LAUNCH_AT_LOGIN_SHIM.m"
}

compile_swift_release() {
    compile_objc_shims
    swiftc -O -target "$TARGET" -sdk "$SDK" \
        -import-objc-header "$OBJC_SHIMS_HEADER" \
        -I Sources/Vorssaint/Services/Audio \
        -I Sources/Vorssaint/Services/SystemMonitor \
        Sources/Vorssaint/**/*.swift \
        build/CoreAudioProcessAPI.o \
        build/ProcessUsageShim.o \
        build/LaunchAtLoginShim.o \
        -o "build/$EXECUTABLE"
}
...
codesign_target() {
    local target="$1"
    if [[ -n "$DEVID" ]]; then
        codesign --force "${CODESIGN_STRIP_XATTRS[@]}" --options runtime --timestamp \
            --entitlements "$ENTITLEMENTS" --sign "$DEVID" "$target"
    elif security find-identity -p codesigning 2>/dev/null | grep -q "$LEGACY_IDENTITY"; then
        codesign --force "${CODESIGN_STRIP_XATTRS[@]}" --sign "$LEGACY_IDENTITY" "$target"
    else
        codesign --force "${CODESIGN_STRIP_XATTRS[@]}" --sign - "$target"
    fi
}
